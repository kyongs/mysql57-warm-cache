...
static monitor_info_t	innodb_counter_info[] =
{
    ...
    {"innodb_warm_buf_pages_stored_stk", "server",
     "Number of stock pages that currently been stored in WARM buffer"
     " (innodb_warm_buf_pages_stored_stk)",
     static_cast<monitor_type_t>(MONITOR_EXISTING | MONITOR_DEFAULT_ON),
     MONITOR_DEFAULT_START, MONITOR_OVLD_SRV_WARM_BUF_PAGES_STORED_STK},

    {"innodb_warm_buf_pages_stored_ol", "server",
     "Number of order-line pages that currently been stored in WARM buffer"
     " (innodb_warm_buf_pages_stored_ol)",
     static_cast<monitor_type_t>(MONITOR_EXISTING | MONITOR_DEFAULT_ON),
     MONITOR_DEFAULT_START, MONITOR_OVLD_SRV_WARM_BUF_PAGES_STORED_OL},

    {"innodb_warm_buf_pages_stored_cust", "server",
     "Number of orders pages that currently been stored in WARM buffer"
     " (innodb_warm_buf_pages_stored_cust)",
     static_cast<monitor_type_t>(MONITOR_EXISTING | MONITOR_DEFAULT_ON),
     MONITOR_DEFAULT_START, MONITOR_OVLD_SRV_WARM_BUF_PAGES_STORED_CUST},

    {"innodb_warm_buf_pages_read_stk", "server",
     "Number of stock pages that been read"
     " (innodb_warm_buf_pages_read_stk)",
     static_cast<monitor_type_t>(MONITOR_EXISTING | MONITOR_DEFAULT_ON),
     MONITOR_DEFAULT_START, MONITOR_OVLD_SRV_WARM_BUF_PAGES_READ_STK},

    {"innodb_warm_buf_pages_read_ol", "server",
     "Number of order-line pages that been read"
     " (innodb_warm_buf_pages_read_ol)",
     static_cast<monitor_type_t>(MONITOR_EXISTING | MONITOR_DEFAULT_ON),
     MONITOR_DEFAULT_START, MONITOR_OVLD_SRV_WARM_BUF_PAGES_READ_OL},

    {"innodb_warm_buf_pages_read_cust", "server",
     "Number of orders pages that been read"
     " (innodb_warm_buf_pages_read_cust)",
     static_cast<monitor_type_t>(MONITOR_EXISTING | MONITOR_DEFAULT_ON),
     MONITOR_DEFAULT_START, MONITOR_OVLD_SRV_WARM_BUF_PAGES_READ_CUST},

    {"innodb_warm_buf_pages_written_stk", "server",
     "Number of stock pages that been written"
     " (innodb_warm_buf_pages_written_stk)",
     static_cast<monitor_type_t>(MONITOR_EXISTING | MONITOR_DEFAULT_ON),
     MONITOR_DEFAULT_START, MONITOR_OVLD_SRV_WARM_BUF_PAGES_WRITTEN_STK},

    {"innodb_warm_buf_pages_written_ol", "server",
     "Number of order-line pages that been written"
     " (innodb_warm_buf_pages_written_ol)",
     static_cast<monitor_type_t>(MONITOR_EXISTING | MONITOR_DEFAULT_ON),
     MONITOR_DEFAULT_START, MONITOR_OVLD_SRV_WARM_BUF_PAGES_WRITTEN_OL},

    {"innodb_warm_buf_pages_written_cust", "server",
     "Number of orders pages that been written"
     " (innodb_warm_buf_pages_written_cust)",
     static_cast<monitor_type_t>(MONITOR_EXISTING | MONITOR_DEFAULT_ON),
     MONITOR_DEFAULT_START, MONITOR_OVLD_SRV_WARM_BUF_PAGES_WRITTEN_CUST},

	{"innodb_move_to_warm_buf_cnt_stk", "server",
     "Number of stock pages that have to move to warm buf"
     " (innodb_move_to_warm_buf_cnt_stk)",
     static_cast<monitor_type_t>(MONITOR_EXISTING | MONITOR_DEFAULT_ON),
     MONITOR_DEFAULT_START, MONITOR_OVLD_SRV_MOVE_TO_WARM_BUF_CNT_STK},

	{"innodb_move_to_warm_buf_cnt_ol", "server",
     "Number of order-line pages that have to move to warm buf"
     " (innodb_move_to_warm_buf_cnt_ol)",
     static_cast<monitor_type_t>(MONITOR_EXISTING | MONITOR_DEFAULT_ON),
     MONITOR_DEFAULT_START, MONITOR_OVLD_SRV_MOVE_TO_WARM_BUF_CNT_OL},

	{"innodb_move_to_warm_buf_cnt_cust", "server",
     "Number of customer pages that have to move to warm buf"
     " (innodb_move_to_warm_buf_cnt_cust)",
     static_cast<monitor_type_t>(MONITOR_EXISTING | MONITOR_DEFAULT_ON),
     MONITOR_DEFAULT_START, MONITOR_OVLD_SRV_STAY_IN_BUF_CNT_CUST},

	{"innodb_stay_in_buf_cnt_stk", "server",
     "Number of stock pages that have stay in original buf"
     " (innodb_stay_in_buf_cnt_stk)",
     static_cast<monitor_type_t>(MONITOR_EXISTING | MONITOR_DEFAULT_ON),
     MONITOR_DEFAULT_START, MONITOR_OVLD_SRV_STAY_IN_BUF_CNT_STK},

	{"innodb_stay_in_buf_cnt_ol", "server",
     "Number of order-line pages that have stay in original buf"
     " (innodb_stay_in_buf_cnt_ol)",
     static_cast<monitor_type_t>(MONITOR_EXISTING | MONITOR_DEFAULT_ON),
     MONITOR_DEFAULT_START, MONITOR_OVLD_SRV_STAY_IN_BUF_CNT_OL},

	{"innodb_stay_in_buf_cnt_cust", "server",
     "Number of customer pages that have stay in original buf"
     " (innodb_stay_in_buf_cnt_cust)",
     static_cast<monitor_type_t>(MONITOR_EXISTING | MONITOR_DEFAULT_ON),
     MONITOR_DEFAULT_START, MONITOR_OVLD_SRV_STAY_IN_BUF_CNT_CUST},

	 {"innodb_warm_buf_pages_written_cp", "server",
     "Number of pages that written from warm buf by cp policy"
     " (innodb_warm_buf_pages_written_cp)",
     static_cast<monitor_type_t>(MONITOR_EXISTING | MONITOR_DEFAULT_ON),
     MONITOR_DEFAULT_START, MONITOR_OVLD_SRV_WARM_BUF_PAGES_WRITTEN_CP},

	 {"innodb_warm_buf_pages_written_lru", "server",
     "Number of pages that written from warm buf by lru policy"
     " (innodb_warm_buf_pages_written_lru)",
     static_cast<monitor_type_t>(MONITOR_EXISTING | MONITOR_DEFAULT_ON),
     MONITOR_DEFAULT_START, MONITOR_OVLD_SRV_WARM_BUF_PAGES_WRITTEN_LRU},
...
#ifdef UNIV_WARM_BUF_CACHE
    /* innodb_warm_buf_pages_stored_stk */
    case MONITOR_OVLD_SRV_WARM_BUF_PAGES_STORED_STK:
        value = srv_stats.warm_buf_pages_stored_stk;
        break;

    /* innodb_warm_buf_pages_stored_ol */
    case MONITOR_OVLD_SRV_WARM_BUF_PAGES_STORED_OL:
        value = srv_stats.warm_buf_pages_stored_ol;
        break;

    /* innodb_warm_buf_pages_stored_cust */
    case MONITOR_OVLD_SRV_WARM_BUF_PAGES_STORED_CUST:
        value = srv_stats.warm_buf_pages_stored_cust;
        break;

    /* innodb_warm_buf_pages_read_stk */
    case MONITOR_OVLD_SRV_WARM_BUF_PAGES_READ_STK:
        value = srv_stats.warm_buf_pages_read_stk;
        break;

    /* innodb_warm_buf_pages_read_ol */
    case MONITOR_OVLD_SRV_WARM_BUF_PAGES_READ_OL:
        value = srv_stats.warm_buf_pages_read_ol;
        break;

    /* innodb_warm_buf_pages_read_cust */
    case MONITOR_OVLD_SRV_WARM_BUF_PAGES_READ_CUST:
        value = srv_stats.warm_buf_pages_read_cust;
        break;

    /* innodb_warm_buf_pages_written_stk */
    case MONITOR_OVLD_SRV_WARM_BUF_PAGES_WRITTEN_STK:
        value = srv_stats.warm_buf_pages_written_stk;
        break;

    /* innodb_warm_buf_pages_written_ol */
    case MONITOR_OVLD_SRV_WARM_BUF_PAGES_WRITTEN_OL:
        value = srv_stats.warm_buf_pages_written_ol;
        break;

    /* innodb_warm_buf_pages_written_cust */
    case MONITOR_OVLD_SRV_WARM_BUF_PAGES_WRITTEN_CUST:
        value = srv_stats.warm_buf_pages_written_cust;
        break;
	
	/* innodb_move_to_warm_buf_cnt_stk */
    case MONITOR_OVLD_SRV_MOVE_TO_WARM_BUF_CNT_STK:
        value = srv_stats.move_to_warm_buf_cnt_stk;
        break;

	/* innodb_move_to_warm_buf_cnt_ol */
    case MONITOR_OVLD_SRV_MOVE_TO_WARM_BUF_CNT_OL:
        value = srv_stats.move_to_warm_buf_cnt_ol;
        break;

	/* innodb_move_to_warm_buf_cnt_cust */
    case MONITOR_OVLD_SRV_MOVE_TO_WARM_BUF_CNT_CUST:
        value = srv_stats.move_to_warm_buf_cnt_cust;
        break;

	/* innodb_stay_in_buf_cnt_stk */
    case MONITOR_OVLD_SRV_STAY_IN_BUF_CNT_STK:
        value = srv_stats.stay_in_buf_cnt_stk;
        break;

	/* innodb_stay_in_buf_cnt_ol */
    case MONITOR_OVLD_SRV_STAY_IN_BUF_CNT_OL:
        value = srv_stats.stay_in_buf_cnt_ol;
        break;

	/* innodb_stay_in_buf_cnt_cust */
    case MONITOR_OVLD_SRV_STAY_IN_BUF_CNT_CUST:
        value = srv_stats.stay_in_buf_cnt_cust;
        break;
	
	case MONITOR_OVLD_SRV_WARM_BUF_PAGES_WRITTEN_CP:
		value = srv_stats.warm_buf_pages_written_cp;
		break;

	case MONITOR_OVLD_SRV_WARM_BUF_PAGES_WRITTEN_LRU:
		value = srv_stats.warm_buf_pages_written_lru;
		break;
